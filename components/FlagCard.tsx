      ...
      <button onClick={() => setOpen((v) => !v)} className="flex w-full items-start gap-3 p-4 text-left">
        ...
        <span className="min-w-0 flex-1">
          <span className="flex flex-wrap items-center gap-2">
            <span className="font-semibold tracking-tight text-slate-100">{flag.clauseTitle}</span>
          ...
          <span className="mt-1 block truncate text-sm leading-relaxed text-slate-400">{flag.plainEnglish}</span>
        </span>
        <ChevronDown className={cn("mt-1 h-4 w-4 shrink-0 text-slate-500 transition", open && "rotate-180")} />
      </button>

      {open && (
        <div className="grid gap-3 border-t border-slate-700/50 p-4 md:grid-cols-2">
          ...
          <div className="rounded-xl border border-slate-700/50 bg-[#0F172A] p-3">
            ...
            <p className="mt-1.5 font-mono text-[13px] leading-relaxed text-slate-200">“{flag.originalText}”</p>
          </div>
          ...
          <div className="rounded-xl border border-slate-700/50 bg-white/[0.03] p-3">
            ...
            <p className="mt-1.5 text-sm leading-relaxed text-slate-200">{flag.plainEnglish}</p>
          </div>
          <div className="rounded-xl border border-[#F59E0B]/30 bg-[#453006]/60 p-3">
            <p className="text-[11px] font-bold uppercase tracking-wider text-[#F59E0B]">Why it matters</p>
            <p className="mt-1.5 text-sm leading-relaxed text-slate-200">{flag.whyItMatters}</p>
          </div>
          <div className="rounded-xl border border-[#6366F1]/40 bg-[#6366F1]/10 p-3">
            ...
            <p className="mt-1.5 font-mono text-[13px] leading-relaxed text-slate-100">{flag.suggestedRevision}</p>
          </div>
        </div>
      )}
    </div>
