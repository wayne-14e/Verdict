...
- `POST /api/analyze?tone=…` — requires `Authorization: Bearer <Firebase ID token>` (`401` otherwise); verifies via Admin SDK (projectId-only init, no SA key needed); **App Check**: when `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` is set, requires valid `X-Firebase-AppCheck` (`403` otherwise); server-side free-quota check when service credentials exist (`402` when exceeded); then Gemini analysis as before (`503` no key, `502/429` AI failure). Whole handler is wrapped so any unexpected crash returns **JSON** `500`, never an HTML error page (prevents the client "Unexpected token '<'" crash on Vercel). Default model **`gemini-3.1-flash-lite`** (lite-only chain — full flash tier stays busy) with per-model fallback (`lib/gemini.ts`: `getGeminiModels()` = `GEMINI_MODEL` → `MODEL_FALLBACKS` = `gemini-3.5-flash-lite`, `gemini-flash-lite-latest` — all verified against the v1beta catalog + live calls; `gemini-1.5-flash` retired/404). `runAcrossModels` enforces a per-attempt fetch timeout (SDK `requestOptions.timeout`) + total budget (52s analyze / 26s counter-offer) so transient 503 high-demand or slow calls never blow the 60s/30s Vercel function cap; budget exhaustion returns `504`, overload returns `503`, quota `429`. Prompts capped at 60k chars.
- `POST /api/counter-offer` — requires valid Bearer ID token (`401` otherwise) + App Check token when configured (`403`); drafts via Gemini (default `gemini-3.5-flash`), deterministic template fallback over real flags only
...
- `app/api/analyze/route.ts`, `app/api/counter-offer/route.ts`, `lib/gemini.ts` (model default + verified fallback chain)
